...
@Service
public class BailTransactionService {
    private static Logger log = LoggerFactory.getLogger(BailTransactionService.class);
    ...
    {
        CaseAlarm caseAlarm = bailService.getNearDaysExpiresCaseAlarm(caseInfo, index,day);
        try {
            if(caseAlarm!=null)
            {
            ...
            log.error("bailservice.caseAlarmService.insertAndUpdate出错",e);
        ...
        if(caseAlarm!=null)
        {
            ...
            caseAlarmPerson = bailService.getOverDateCaseAlarmPerson(caseAlarm, caseInfo, new UserInfo(), false);
            try {
                ...
                log.error("bailservice.caseAlarmPersonService.insertAndUpdate出错",e);
                TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
            }
        ...
        if(caseAlarm!=null)
        {
            ...
            try {
                ...
                log.error("bailservice.userInfoService.getManager出错",e);
            }
            if (userInfos != null && userInfos.size() > 0)
            {
                ...
                {
                    try {
                        CaseAlarmPerson caseAlarmManager = bailService.getOverDateCaseAlarmPerson(caseAlarm, caseInfo, userInfo, true);
        ...
        {
            try {
                CaseFlowRecord caseFlowRecord = bailService.insertToCaseFlowRecord(caseInfo);
                if(caseFlowRecord!=null)
                {
                    caseFlowRecordService.insertAndUpdate(caseFlowRecord);//插入后更新caseFlowRecord中caseAlarm表中id
                }
            } catch (Exception e) {
                log.error("bailservice.caseFlowRecordService.insertAndUpdate出错",e);
                TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
            }
        }
    }
